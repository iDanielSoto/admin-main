import { Users, Edit, Trash2, UserCheck, Plus } from 'lucide-react';
  ...
  <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 h-full">
    ...
    <div className="space-y-2">
      {employees.map((employee) => (
        <div key={employee.id} className="flex items-center justify-between bg-gray-50 p-2 rounded hover:bg-gray-100 transition">
          <div className="flex items-center">
            <UserCheck className="w-5 h-5 mr-2 text-green-500" />
            <span>{employee.name}</span>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={() => onEdit(employee)} className="text-blue-500 hover:text-blue-600">
              <Edit className="w-4 h-4" />
            </button>
            <button onClick={() => onDelete(employee.id)} className="text-red-500 hover:text-red-600">
